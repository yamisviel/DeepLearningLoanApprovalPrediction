...
🚀 Workflow Overview
    ...
    Training: Deep Learning Loan Approval.py processes the data and trains a multi-layer perceptron. It uses a 99th percentile cap for income to handle extreme outliers and applies StandardScaler logic.

    Persistence: The script saves only the "Best" version of the model based on validation loss, alongside a .pkl file containing the exact scaling parameters used.

    Inference: Loan Approval Model Loading.py ingests new CSV data, uses reindex to ensure the schema matches the training set, and outputs classification results at a custom probability threshold (default: 0.3).
